...
variable {Ω S : Type*} [ProbabilitySpace Ω] [Fintype S] {X : Ω → S}
...
lemma entropy_zero  (hΩ : ¬ProbabilitySpace.isNondeg Ω) (X : Ω → S) : H[X] = 0 := by
  unfold entropy
  conv =>
    lhs; congr; rfl; ext s
...
/-- The Jensen bound -/
lemma entropy_le_log (hX : Measurable X): H[X] ≤ log (Fintype.card S) := by
