...
$$ |H_1| \leq 2|S|^{12}.$$
By hypothesis, $A$ is covered by at most $2|S|^{12}$ translates of $H$, and hence by at most $4|S|^{24}$ translates of $\{ (x,\phi(x)): x \in G \}$.  As $\phi$ is a homomorphism, each such translate can be written in the form $\{ (x,\phi(x)+c): x \in G \}$ for some $c \in G'$, and the claim follows.
\end{proof}
