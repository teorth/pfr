...
\begin{proposition}[Existence of $\tau$-minimizer]\label{multiTau_continuous, tau-min-exist-multi}\lean{multiTau_min_exists}\leanok  If $G$ is finite, then a $\tau$-minimizer exists.
\end{proposition}
...
\begin{lemma}[Distance bound]\label{xi-z2-w-dist}\lean{sum_of_conditional_distance_le}\leanok We have $\sum_{i=1}^m d[X_i;Z_2|W] \leq 8(m^3-m^2) k$.
\end{lemma}
...
\begin{proposition}[Vanishing entropy]\label{k-vanish}\leanok  We have $k = 0$.
\end{proposition}
