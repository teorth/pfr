...
\begin{proof}\uses{alternative-mutual, submodularity, relabeled-entropy}\leanok
  By \Cref{alternative-mutual} it suffices to show that $\bbH[Y|X] \leq \bbH[Y|f(X)]$. But this follows from Corollary \ref{submodularity} (and \Cref{relabeled-entropy}).
\end{proof}
...
\begin{proof}\uses{data-process-unc, conditional-mutual-def}  Apply \Cref{data-process-unc} to $X,Y$ conditioned to the event $Z=z$, multiply by ${\bf P}[Z=z]$, and sum using Definition \ref{conditional-mutual-def}.
\end{proof}
...
Since $X_3+Y_3$ is a function of $X_3,Y_3$, we see from \Cref{relabeled-entropy} and Corollary \ref{add-entropy} that
...
and thus by \Cref{ruz-indep}, \Cref{ruz-copy}, \Cref{relabeled-entropy}, Corollary \ref{add-entropy}
...
By \Cref{ruz-copy} we may take the $X_i$ to be jointly independent.  From Corollary \ref{sumset-lower}, we see that for any distinct $1 \leq j,k \leq m$, we have
  \[
...
and hence by Definition \ref{tau-def-multi} and \Cref{multidist-nonneg}
...
\begin{proof}\uses{multidist-lower, cond-multidist-def, cond-multidist-alt}
  Immediate from \Cref{multidist-lower}, \Cref{cond-multidist-alt}, and Definition \ref{cond-dist-def}.
\end{proof}
...
\begin{lemma}[Multidistance chain rule]\label{multidist-chain-rule}  Let $\pi \colon G \to H$ be a homomorphism of abelian groups and let $X_{[m]}$ be a tuple of jointly independent $G$-valued random variables.  Then $D[X_{[m]}]$ is equal to
  ...
  Now we apply \Cref{multidist-chain-rule-iter} with $I = \{1,\dots, m\}$ and $X_i :=  (X_{i,j})_{j = 1}^m$.  Using joint independence and Corollary \ref{add-entropy}, we find that
  \[
...
\begin{proof}\uses{key, data-process}
  ...
  Since $Z_1$ is a function of $(P_i)_{i \in \Z/m\Z}$ by~\eqref{eq:is-fn}, and similarly $Z_2$ is a function of $(Q_j)_{j \in \Z/m\Z}$, it follows immediately from \Cref{data-process} that
  \[
    \bbI[ Z_1 : Z_2 \,|\, W ] \leq t.
  \]
 ...
 Similarly, the column sums for $j \in \{1,\dots, m\}$ are
  ...
  Hence, using~\eqref{eq:is-fn} and \Cref{data-process} again, \Cref{key} tells us
  \[
...
\begin{proof}\uses{main-entropy, unif-exist, uniform-entropy-II, jensen-bound,ruz-dist-def,ruzsa-diff,bound-conc,ruz-cov}  Repeat the proof of \Cref{pfr_aux}, but with Theorem \ref{main-entropy} in place of Theorem \ref{entropy-pfr}.
\end{proof}
...
\begin{theorem}[PFR]\label{pfr-torsion}  Suppose that $G$ is a finite abelian group of torsion $m$.
  ...
  \begin{proof}\uses{pfr_aux_torsion} Repeat the proof of Theorem \ref{pfr}, but with \Cref{pfr_aux_torsion} in place of \Cref{pfr_aux}.
  \end{proof}
