...
\begin{proof}\uses{multidist-def} Clear from Lemma \ref{copy-ent}.
\end{proof}
...
\begin{proposition}[Existence of $\tau$-minimizer]\label{tau-min-exist-multi}\lean{multiTau_min_exists}\leanok  If $G$ is finite, then a $\tau$-minimizer exists.
\end{proposition}
...
\begin{lemma}[Multidistance chain rule]\label{multidist-chain-rule}\lean{multiDist_chainRule}\uses{multidist-def}  Let $\pi \colon G \to H$ be a homomorphism of abelian groups and let $X_{[m]}$ be a tuple of jointly independent $G$-valued random variables.  Then $D[X_{[m]}]$ is equal to
  \begin{equation}
...
\begin{lemma}[Zero-sum]\label{Zero-sum}\lean{sum_of_z_eq_zero} We have
  \begin{equation}%
    \label{eq:sum-zero}
    Z_1+Z_2+Z_3= 0
  \end{equation}
...
\begin{proposition}[Mutual information bound]\label{prop:52}\lean{mutual_information_le_t}
  We have
  \[
    \bbI[Z_1 : Z_2\, |\, W],\
    \bbI[Z_2 : Z_3\, |\, W],\
...
\begin{lemma}[Entropy of $W$]\label{ent-w}\lean{entropy_of_W_le}  We have $\bbH[W] \leq (2m-1)k + \frac1m \sum_{i=1}^m \bbH[X_i]$.
\end{lemma}
...
\begin{lemma}[Entropy of $Z_2$]\label{ent-z2}\lean{entropy_of_Z_two_le}  We have $\bbH[Z_2] \leq (8m^2-16m+1) k + \frac{1}{m} \sum_{i=1}^m \bbH[X_i]$.
\end{lemma}
...
\begin{lemma}[Mutual information bound]\label{mutual-w-z2}\lean{mutual_of_W_Z_two_le}  We have  $\bbI[W : Z_2] \leq 2 (m-1) k$.
\end{lemma}
...
\begin{lemma}[Distance bound]\label{xi-z2-w-dist}\lean{sum_of_conditional_distance_le} We have $\sum_{i=1}^m d[X_i;Z_2|W] \leq 8(m^3-m^2) k$.
\end{lemma}
...
\begin{lemma}[Application of BSG]
  \label{lem:get-better}\lean{dist_of_U_add_le}
...
\begin{theorem}[Entropy form of PFR]\label{main-entropy}\lean{dist_of_X_U_H_le} Suppose that $G$ is a finite abelian group of torsion $m$.  Suppose that $X$ is a $G$-valued random variable. Then there exists a subgroup $H \leq G$ such that \[ d[X;U_H] \leq 64 m^3 d[X;X].\]
\end{theorem}
...
\begin{theorem}[PFR]\label{pfr-torsion}\lean{torsion_PFR}  Suppose that $G$ is a finite abelian group of torsion $m$.
