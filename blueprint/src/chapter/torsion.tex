...
\begin{proof}\uses{data-process-unc-one, entropy-comm}
...
\begin{lemma}[Comparing sums]\label{compare-sums}\lean{ent_of_sum_le_ent_of_sum}  Let $(X_i)_{i \in I}$ and $(Y_j)_{j \in J}$ be tuples of independent random variables, and let $f: J \to I$ be a function, then
  $$ \bbH[\sum_{j \in J} Y_j] \leq \bbH[ \sum_{i \in I} X_i ] + \sum_{j \in J} (\bbH[ Y_j - X_{f(j)}] - \bbH[X_{f(j)}]).$$
\end{lemma}

\begin{proof}\uses{klm-1, kv, neg-ent, sumset-lower}  Write $W := \sum_{i \in I} X_i$.  From Corollary \ref{sumset-lower} we have
...
\begin{definition}[Multidistance]\label{multidist-def}\lean{multiDist}  Let $X_I = (X_i)_{i \in I}$ non-empty finite tuple of $G$-valued random variables $X_i$. Then we define
\[
...
\begin{lemma}[Nonnegativity]\label{multidist-nonneg}\lean{multiDist_nonneg}  For any such tuple, we have $D[X_I] \geq 0$.
\end{lemma}
...
\begin{lemma}[Relabeling]\label{multidist-perm}\lean{multiDist_of_perm} If $\phi: J \to I$ is a bijection, then $D[X_I] = D[(X_{\phi(j)})_{j \in J}]$.
\end{lemma}
...
\begin{proof}\uses{multidist-ruzsa-I, ruzsa-nonneg, lem:100pc}  From Lemma \ref{multidist-ruzsa-I} and Lemma \ref{ruzsa-nonneg} we have $d[X_j; X_{-k}]=0$ for all $j,k \in I$.  The claim now follows from Lemma \ref{lem:100pc}.
\end{proof}
...
\begin{definition}[Conditional multidistance]\label{cond-multidist-def}\uses{multidist-def} If $X_I = (X_i)_{i \in I}$ and $Y_I = (Y_i)_{i \in I}$ are tuples of random variables, with the $X_i$ being $G$-valued (but the $Y_i$ need not be), then we define
...
\begin{lemma}[Multidistance chain rule]\label{multidist-chain-rule}  Let $\pi \colon G \to H$ be a homomorphism of abelian groups and let $X_{I}$ be a tuple of jointly independent $G$-valued random variables.  Then $D[X_{I}]$ is equal to
  ...
  \begin{proof}\uses{conditional-mutual-alt, relabeled-entropy, chain-rule} For notational brevity during this proof, write $S := \sum_{i \in I} X_i$.
