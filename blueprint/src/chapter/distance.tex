...
\begin{proof} \uses{sumset-lower-gen} This follows from Lemma \ref{sumset-lower-gen} by conditioning to $Z = z$ and summing over $z$ (weighted by $ \bbP[Z=z]$).
\end{proof}
...
\begin{proof}\uses{submodularity, add-entropy, relabeled-entropy}
...
\begin{definition}[Conditionally independent trials]\label{cond-trial}
  \lean{ProbabilityTheory.condIndepFun, ProbabilityTheory.IdentDistrib}\leanok
    Let $X,Y$ be random variables on a space $\Omega$.
...
$$ \bbH[X_1,X_2,Y] = 2 \bbH[X] + \bbH[Y] + 2  \bbI[X:Y] = 2 \bbH[X,Y] - \bbH[Y].$$
\end{lemma}

\begin{proof} \uses{chain-rule, add-entropy, information-def} We calculate
  \begin{equation}
  \begin{split} \bbH[X_1, X_2, Y'] &= \bbH[X_1,X_2|Y] + \bbH[Y] \\
    &= 2 \bbH[X|Y] + \bbH[Y] \\
    &= 2 \bbH[X] + \bbH[Y] + 2  \bbI[X:Y].
...
\begin{equation}\label{bsg-24} \bbH[A_1 - B_2, A_1, B_1] = \bbH[A_1, B_1, A_2, B_2] = 2\bbH[A,B] - \bbH[Z].\end{equation}
Next observe that
...
\begin{lemma}\label{first-useful}
  \uses{independent-def, cond-dist-def}
...
\begin{lemma}\label{second-useful}
  \uses{independent-def, cond-ruz-def}
  \lean{condDist_diff_ofsum_le}
