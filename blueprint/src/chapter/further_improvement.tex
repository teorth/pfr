...
\begin{lemma}[Rho of subgroup]\label{rho-subgroup}\uses{rho-def}\lean{rho_of_subgroup}  If $H$ is a finite subgroup of $G$, and $\rho(U_H) \leq r$, then there exists $t$ such that $|A \cap (H+t)| \geq 2^{-r} \sqrt{|A||H|}$, and $|H|/|A|\in[2^{-2r},2^{2r}]$.
\end{lemma}
...
\begin{lemma}[Rho invariant]\label{rho-invariant}\lean{rho_of_translate}\leanok  For any $s \in G$, $\rho(X+s) = \rho(X)$.
\end{lemma}
...
\begin{lemma}\label{rho-cond-invariant}\lean{condRho_of_translate}\leanok
...
\begin{lemma}\label{rho-cond-relabeled}\lean{condRho_of_injective}\leanok
...
\begin{proof}\uses{rho-cts} Clear from compactness.
\end{proof}
...
\begin{lemma}\label{I1-I2-diff}\lean{dist_add_dist_eq}\leanok
