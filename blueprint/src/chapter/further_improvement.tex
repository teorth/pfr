...
\begin{definition}[Rho plus]\label{rhoplus-def}\lean{rho_plus}\ises{rhominus-def}\leanok  For any $G$-valued random variable $X$, we define $\rho^+(X) := \rho^-(X) + \bbH(X) - \bbH(U_A)$.
\end{definition}
