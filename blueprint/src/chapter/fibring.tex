...
By \Cref{fibring-ident} and the nonnegativity of conditional Ruzsa distance (from Lemma \ref{ruzsa-nonneg}) we have
...
\begin{proof}
  ...
  We apply \Cref{fibring-ident} with $H := G \times G$, $H' := G$, $\pi$ the addition homomorphism $\pi(x,y) := x+y$, and with the random variables $Z_1 := (Y_1,Y_3)$ and $Z_2 := (Y_2,Y_4)$.  Then by independence (Lemma \ref{add-entropy})
\[
