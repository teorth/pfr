...
\begin{definition}[Additive energy]\label{energy-def}  If $G$ is a group, and $A$ is a finite subset of $G$, the \emph{additive energy} $E(A)$ of $A$ is the number of quadruples $(a_1,a_2,a_3,a_4) \in A^4$ such that $a_1+a_2 = a_3+a_4$.
\end{definition}

\begin{lemma}[Cauchy--Schwarz bound]\label{cs-bound}\uses{energy-def}  If $G$ is a group, $A,B$ are finite subsets of $G$, then
...
\begin{lemma}[Balog--Szemer\'edi--Gowers lemma]\label{bsg}\uses{energy-def} Let $G$ be an abelian group, and let $A$ be a finite non-empty set with $E(A) \geq |A|^3 / K$ for some $K \geq 1$.  Then there is a subset $A'$ of $A$ with $|A'| \geq |A| / (C_1 K^{C_2})$ and $|A'+A'| \leq C_3 K^{C_4} |A'|$, where
$$ C_1 = ???, C_2 = ???, C_3 = ???, C_4 = ???.$$
\end{lemma}
...
\begin{theorem}[Approximate homomorphism form of PFR]\label{approx-hom-pfr} Let $G,G'$ be finite abelian $2$-groups.
...
\begin{proof}\uses{goursat, cs-bound, bsg} Consider the graph $A \subset G \times G'$ defined by
$$ A := \{ (x,f(x)): x \in G \}.$$
