...
\begin{lemma}[Counting]\label{gcount} Let $G$ be a finite abelian $2$-group, and let $x \in G$ be non-zero.  Then there are $|G|/2$ homomorphisms $\phi: G \to \Z/2\Z$ such that $\phi(x) = 1$.
\end{lemma}

\begin{proof}\uses{gdual}  The map $\phi \mapsto \phi(x)$ is a homomorphism from $\mathrm{Hom}(G,\Z/2\Z)$ to $\Z/2\Z$, and by Lemma \ref{gdual} the kernel has order equal to the order of $G / \{0,x\}$, which is $|G|/2$.  Then the preimage of $1$ must also be of order $|G|/2$.
\end{proof}

\begin{lemma}[Slicing]\label{gslice}  Let $G$ be a finite abelian $2$-group, and let $A$ be a subset of $G$.  Then there exists a homomorphism $\phi: G \to \Z/2\Z$ such that $|A \cap \phi^{-1}(1)| \geq (|A|-1)/2$.
\end{lemma}

\begin{proof}\uses{gdual, gcount}  We have
...
\begin{corollary}[Approximate homomorphism form of PFR, no constant term]\label{approx-hom-pfr-no-const} Let $G,G'$ be finite abelian $2$-groups.
  Let $f: G \to G'$ be a function, and suppose that there are at least $|G|^2 / K$ pairs $(x,y) \in G^2$ such that
$$ f(x+y) = f(x) + f(y).$$
...
\begin{proof}\uses{approx-hom-pfr, gslice} By Theorem \ref{approx-hom-pfr}, there exists a homomorphism $\phi: G \to G'$ and a constant $c \in G'$ such that the set $A := \{ x \in G: f(x) = \phi(x) \}$ has cardinality at least $|G| / (2 ^ {172} * K ^ {146})$.  By Lemma \ref{gslice}, there exists a homomorphism $\phi': G \to \Z/2\Z$ such that
