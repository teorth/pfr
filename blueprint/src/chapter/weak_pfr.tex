...
\begin{lemma}\label{app-ent-pfr}\lean{app_ent_PFR}
...
\end{lemma}
\begin{proof}
\uses{entropy-pfr, ruzsa-diff, dist-projection, ruzsa-nonneg}
By Theorem \ref{entropy-pfr} there exists a subgroup $H$ such that $d[X;U_H]\leq 6d[X;Y]$ and $d[Y;U_H]\leq 6d[X;Y]$. Using Lemma \ref{dist-projection} we deduce that $\mathbb{H}(\psi(X))\leq 12 d[X;Y]$ and $\mathbb{H}(\psi(Y))\leq 12d[X;Y]$. The second claim follows adding these inequalities and using the assumption on $\mathbb{H}(X)+\mathbb{H}(Y)$.

Furthermore we have by Lemma \ref{ruzsa-diff}
\[\log \lvert H \rvert-\mathbb{H}(X)\leq 2d[X;U_H]\leq 12d[X;Y]\]
...
\begin{lemma}\label{pfr-projection}\lean{PFR_projection}
...
\[\mathbb{H}(\psi(X))+\mathbb{H}(\psi(Y))> 48d[\psi(X);\psi(Y)]\]
and
...
\begin{lemma}\label{single-fibres}\lean{single_fibres}
...
\begin{definition}\label{dimension-def}\lean{dimension}
...
\begin{theorem}\label{weak-pfr-asymm}\lean{weak_PFR_asymm}
...
and
\[\mathbb{H}(\tilde{\phi}(U_A))+\mathbb{H}(\tilde{\phi}(U_B))\leq 48 d[\tilde{\phi}(U_A);\tilde{\phi}(U_B)]\]
...
\[\log \frac{\lvert A\rvert\lvert B\rvert}{\lvert A_x\rvert\lvert B_y\rvert}\leq 48(d[U_A;U_B]-d[U_{A_x};U_{B_y}]).\]
...
\[\log \frac{\lvert A_x\rvert\lvert B_y\rvert}{\lvert A'\rvert\lvert B'\rvert}\leq 48d[U_{A_x};U_{B_y}].\]
...
\begin{theorem}\label{weak-pfr-symm}\lean{weak_PFR_symm}
...
\begin{theorem}\label{weak-pfr-int}\lean{weak_PFR_int}
