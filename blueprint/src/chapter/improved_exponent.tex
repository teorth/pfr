...
\begin{definition}[New definition of $\eta$]\label{eta-def-new}  $\eta$ is a real parameter with $\eta > 0$.
\end{definition}
...
\begin{lemma}[Constructing good variables, III']\label{averaged-construct-good}
  One has
...
\begin{lemma}[General inequality]\label{gen-ineq}  Let $X_1, X_2, X_3, X_4$ be independent $G$-valued random variables, and let $Y$ be another $G$-valued random variable.  Set $S := X_1+X_2+X_3+X_4$. Then
  \begin{align*}
...
\begin{proof}\uses{cond-dist-fact, first-useful, cor-fibre}
...
\begin{lemma}[Bound on distance differences]\label{dist-diff-bound} We have
\begin{align*}  &\sum_{i=1}^2 \sum_{A,B \in \{U,V,W\}: A \neq B}  d[X_i^0;A|B, S] - d[X_i^0;X_i]\\
&\qquad \leq 12 k + \frac{4(2 \eta k - I_1)}{1-\eta}.
\end{align*}
\end{lemma}

\begin{proof}\uses{gen-ineq, relabeled-entropy-cond,second-estimate}  If we apply Lemma \ref{gen-ineq} with $X_1:=X_1$, $Y:=X_1^0$ and $(X_2,X_3,X_4)$ equal to the $3!$ permutations of $(X_2,\tilde X_1,\tilde X_2)$, and sums (using the symmetry $\bbH[X|X+Y] = \bbH[Y|X+Y]$, which follows from Lemma \ref{relabeled-entropy-cond}), we can bound
...
\begin{theorem}[Improved $\tau$-decrement]\label{de-prop'}
Suppose $0 < \eta < 1/8$.  Let $X_1, X_2$ be tau-minimizers.  Then $d[X_1;X_2] = 0$.
\end{theorem}

\begin{proof}\uses{averaged-construct-good, dist-diff-bound, uvw-s}  From Lemma \ref{averaged-construct-good}, Lemma \ref{dist-diff-bound}, and Lemma \ref{uvw-s} one has
...
\begin{theorem}[Improved entropy version of PFR]\label{entropy-pfr'}
...
\begin{proof}\uses{entropy-pfr', unif-exist, uniform-entropy-II, jensen-bound,ruz-dist-def,ruzsa-diff,bound-conc,ruz-cov} By repeating the proof of Theorme \ref{pfr} and using Theorem \ref{entropy-pfr'} one can obtain the claim with $11$ replaced by $3 +\frac{1}{\eta}$ for any $0 < \eta < 1/8$.  Now send $\eta$ to $1/8$ and use the fact that there are only finitely many possible choices for the subspace $H$ to conclude.
\end{proof}

Of course, by replaccing Theorem \ref{pfr} with Theorem \ref{pfr'} we may also improve constants in downstream theorems in a straightforward manner.
