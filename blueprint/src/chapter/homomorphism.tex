...
\begin{lemma}[Hahn-Banach type theorem]\label{hb-thm}  Let $H_0$ be a subgroup of $G$.  Then every homomorphism $\phi: H_0 \to G'$ can be extended to a homomorphism $\tilde \phi: G \to G'$.
\end{lemma}
...
\begin{lemma}[Goursat type theorem]\label{goursat}  Let $H$ be a subgroup of $G \times G'$.  Then there exists a subgroup $H_0$ of $G$, a subgroup $H_1$ of $G'$, and a homomorphism $\phi: G \to G'$ such that
...
\begin{theorem}[Homomorphism form of PFR]\label{hom-pfr}  Let $f: G \to G'$ be a function, and let $S$ denote the set
