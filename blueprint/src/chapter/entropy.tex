...
\begin{lemma}[Entropy and relabeling]\label{relabeled-entropy}\lean{ProbabilityTheory.entropy_of_inj_map} \uses{entropy-def}  If $X: \Omega \to S$ and $Y: \Omega \to T$ are random variables, and $Y = f(X)$ almost surely for some injection $f: S \to T$, then $H[X] = H[Y]$.
\end{lemma}
...
\begin{lemma}[Entropy of uniform random variable]\label{uniform-entropy}
  \uses{entropy-def}
  \lean{ProbabilityTheory.entropy_eq_log_card}
...
\begin{lemma}[Symmetry of joint entropy]\label{joint-symm}
  \uses{entropy-def}
  \lean{ProbabilityTheory.jointEntropy_symm}
...
\begin{definition}[Conditional entropy]\label{conditional-entropy-def}
  ...
  \lean{ProbabilityTheory.condEntropy_def}\leanok
...
\begin{lemma}[Conditional entropy and relabeling]\label{relabeled-entropy-cond}
  ...
  \lean{ProbabilityTheory.condEntropy_of_inj_map}
...
\begin{lemma}[Chain rule]\label{chain-rule}
  \uses{conditional-entropy-def}
  \lean{ProbabilityTheory.chain_rule}\leanok
...
\begin{definition}[Mutual information]\label{information-def}
  \lean{mutualInformation}\leanok
...
\begin{lemma}[Alternative formulae for mutual information]\label{alternative-mutual}\uses{information-def, conditional-entropy-def, entropy-def}
  \lean{mutualInformation_eq_entropy_sub_condEntropy} \leanok
...
\begin{proof}
  \uses{joint-symm,chain-rule}\leanok
...
\begin{corollary}[Subadditivity]\label{subadditive}
  \uses{entropy-def}
  \lean{ProbabilityTheory.jointEntropy_le_sum}
...
\begin{corollary}[Conditioning reduces entropy]\label{cond-reduce}
...
\begin{corollary}[Submodularity]\label{submodularity}
  \uses{conditional-entropy-def}
  \lean{ProbabilityTheory.entropy_submodular}
