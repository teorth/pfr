...
\begin{lemma}[Symmetry of joint entropy]\label{joint-symm}\uses{entropy-def}  If $X: \Omega \to S$ and $Y: \Omega \to T$ are random variables, then $H[X,Y] = H[ Y,X]$.\end{lemma}
...
\begin{lemma}[Conditional chain rule]\label{conditional-chain-rule} \uses{conditional-entropy-def}
...
\begin{corollary}[Subadditivity]\label{subadditive}\uses{entropy-def}  With notation as above, we have $H[X,Y] \leq H[X] + H[Y]$.
\end{corollary}
...
\begin{corollary}[Submodularity]\label{submodularity} \uses{conditional-entropy-def} With three random variables $X,Y,Z$, one has $H[X|Y,Z] \leq H[X|Z]$.
\end{corollary}
