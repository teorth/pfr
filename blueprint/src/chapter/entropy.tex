...
\begin{proof}
  ...
  For the first part, use Definition \ref{conditional-entropy-def} and then \Cref{relabeled-entropy}.  The second part is a direct computation.
\end{proof}
...
\begin{proof}  \uses{submodularity,chain-rule} \leanok Apply Corollary \ref{submodularity} and \Cref{chain-rule}.
\end{proof}
...
\uses{submodularity}
\leanok
Use Definition \ref{conditional-mutual-def} and \Cref{submodularity}.
\end{proof}
...
\begin{proof} \uses{vanish-entropy, conditional-independent-def}\leanok Immediate from \Cref{vanish-entropy} and Definition \ref{conditional-independent-def}.
\end{proof}
