...
\begin{lemma}[$\tau$ depends only on distribution]\label{ProbabilityTheory.IdentDistrib.tau_eq}\leanok
  \uses{tau-def}
  \lean{tau_copy}  If $X'_1, X'_2$ are copies of $X_1,X_2$, then $\tau[X'_1;X'_2] = \tau[X_1;X_2]$.
\end{lemma}
...
\begin{lemma}[Distance lower bound]\label{distance-lower}
  \uses{tau-minimizes, ruz-dist-def}\leanok
...
\begin{lemma}[First estimate]\label{first-estimate}
  \lean{first_estimate} We have $I_1 \leq 2 \eta k$.
\end{lemma}

\begin{proof}\uses{cor-fibre, distance-lower, cond-distance-lower, first-useful}  From Corollary \ref{cor-fibre} we have
  \begin{align}
...
\begin{equation}  d[X_1^0;X_1|X_1+\tilde X_2] - \dist[X_1^0;X_1] \leq \tfrac{1}{2} k + \tfrac{1}{4} \bbH[X_1] - \tfrac{1}{4} \bbH[X_2]                                 \label{second-dist1}
\end{equation}
and
\begin{equation}
  \label{second-dist2}
...
\begin{lemma}[Bound on distance increments]\label{total-dist}
...
\begin{proof}
  ...
  \[
    d[X^0_1;U|S] - d[X^0_1;X_1] \leq \tfrac{1}{2} (H[S] -  \bbH[X_1]).
  \]
  Further applications of Lemma \ref{second-useful} give
  \begin{align*}
  d[X^0_2;U|S] - d[X^0_2; X_2] &\leq \tfrac{1}{2} (H[S] -  \bbH[X_2]) \\
  d[X^0_1;V|S] - d[X^0_1;X_1] &\leq \tfrac{1}{2} (H[S] -  \bbH[X_1])\\
  d[X^0_2;V|S] - d[X^0_2;X_2] &\leq \tfrac{1}{2} (H[S] -  \bbH[X_2])
  \end{align*}
  and
  \[ d[X^0_1;W|S] - d[X^0_1;X_1] \leq \tfrac{1}{2} (H[S] + \bbH[W] - \bbH[X_1] - \bbH[W']),\] where $W' := X_2 + \tilde X_2$.
  ...
  \[ d[X^0_2;W'|S] - d[X^0_2;X_2] \leq \tfrac{1}{2} (H[S] + \bbH[W'] - \bbH[X_2] - \bbH[W]).\]
...
\begin{proof}\uses{lem-bsg, cond-dist-fact}
  ...
  \begin{align*} & 2( \bbH[T_1] + \bbH[T_2] + \bbH[T_3]) - 3 \bbH[T_1,T_2] \\ & = 2(H[T_1] + \bbH[T_2] + \bbH[T_3]) - \bbH[T_1,T_2] - \bbH[T_2,T_3] - \bbH[T_1, T_3] = \delta,\end{align*}
...
\begin{proof}
  \uses{construct-good, cond-distance-lower, key-ident, uvw-s, total-dist, first-est}
