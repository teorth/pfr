...
For the next two lemmas, let $G=\F_2^n$ and let $(T_1,T_2,T_3)$ be a $G^3$-valued random variable such that $T_1+T_2+T_3=0$ holds identically. Set
...
\begin{lemma}[Constructing good variables, I]\label{construct-good-prelim}
  \lean{construct_good-prelim}\leanok
  One has
  \begin{align*}  k \leq
...
\begin{proof}\uses{lem-bsg, cond-dist-fact,distance_lower}
...
\begin{proof}\uses{construct-good-prelim}
...
\begin{proof}
  \uses{construct-good, key-ident, uvw-s, total-dist, first-estimate, eta-def}
  We take contrapostives and assume that $X_1,X_2$ are tau-minimizers, and wish to show that $k := d[X_1;X_2]$ vanishes.  Applying Lemma \ref{construct-good} with any random variables $(T_1,T_2,T_3)$ such that $T_1+T_2+T_3=0$ holds identically, we deduce that
\[
