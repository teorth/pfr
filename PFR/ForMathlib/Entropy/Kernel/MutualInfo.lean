...
lemma mutualInfo_nonneg [Nonempty S] [Countable T] [Nonempty U]
    {κ : Kernel T (S × U)} {μ : Measure T} [IsFiniteMeasure μ]
...
lemma mutualInfo_compProd
  ...
  have : Nonempty U := nonempty_of_isMarkovKernel η
  rw [mutualInfo, entropy_compProd hκ hη, fst_compProd]

variable [Countable U]

lemma mutualInfo_eq_fst_sub [Nonempty S] {κ : Kernel T (S × U)} [IsMarkovKernel κ]
...
lemma entropy_condKernel_le_entropy_snd [Nonempty S] {κ : Kernel T (S × U)} [IsMarkovKernel κ]
    {μ : Measure T} [IsProbabilityMeasure μ] [FiniteSupport μ]
    (hκ : AEFiniteKernelSupport κ μ) :
...
variable [Nonempty S] [Nonempty U]
