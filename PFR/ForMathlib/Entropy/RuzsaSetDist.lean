...
lemma rdist_set_nonneg (A B: Set G) [Finite A] [Finite B] [Nonempty A] [Nonempty B] :
    0 ≤ dᵤ[A # B] := by
...
lemma rdist_set_symm (A B: Set G) [Finite A] [Finite B]  [Nonempty A] [Nonempty B] :
    dᵤ[A # B] = dᵤ[B # A] := by
...
lemma rdist_set_triangle (A B C : Set G) [Finite A] [Finite B] [Finite C]
...
lemma rdist_set_add_const (A B: Set G) [Finite A] [Finite B] [Nonempty A] [Nonempty B] (c c' : G) :
    dᵤ[A + {c} # B + {c'}] = dᵤ[A # B] := by
  obtain ⟨Ω, mΩ, UA, hμ, hUA_mes, hUA_unif, -, hUA_fin ⟩ := exists_isUniform_measureSpace' A
  obtain ⟨Ω', mΩ', UB, hμ', hUB_mes, hUB_unif, -, hUB_fin ⟩ := exists_isUniform_measureSpace' B
...
lemma rdist_set_of_inj (A B: Set G) [Finite A] [Finite B] [Nonempty A] [Nonempty B]
