...
lemma single_fibres {G H Ω Ω': Type u}
  ...
  haveI h_Ax (x : X) : Nonempty (A_ x.val) := by
    ...
    use UA ω
    exact Set.mem_inter (hUA_mem ω) (by exact hω)
  haveI h_By (y : Y): Nonempty (B_ y.val) := by
    obtain ⟨ω, hω⟩ := (FiniteRange.mem_iff _ _).mp y.property
    use UB ω
    exact Set.mem_inter (hUB_mem ω) (by exact hω)
  let p (x : H) (y : H) : ℝ := (Nat.card (A_ x)) * (Nat.card (B_ y)) / ((Nat.card A) * (Nat.card B))
  ...
  calc
    _ = d[UA | φ.toFun ∘ UA # UB | φ.toFun ∘ UB] := by
      ...
      have h_μx : IsProbabilityMeasure μx := by
        apply ProbabilityTheory.cond_isProbabilityMeasure (ℙ : Measure Ω)
        rewrite [Set.preimage_comp, hUA_coe.measure_preimage _ hUA', Set.inter_comm]
        simp? [← Nat.pos_iff_ne_zero] says
          simp only [measure_univ, Set.Finite.coe_toFinset, ZeroHom.toFun_eq_coe,
            AddMonoidHom.toZeroHom_coe, one_mul, Set.Finite.mem_toFinset, ne_eq,
            ENNReal.div_eq_zero_iff, Nat.cast_eq_zero, ENNReal.nat_ne_top, or_false,
            ← Nat.pos_iff_ne_zero, Nat.card_pos]
      have h_μy : IsProbabilityMeasure μy := by
        apply ProbabilityTheory.cond_isProbabilityMeasure (ℙ : Measure Ω')
        rewrite [Set.preimage_comp, hUB_coe.measure_preimage _ hUB', Set.inter_comm]
        simp? [← Nat.pos_iff_ne_zero] says
          simp only [measure_univ, Set.Finite.coe_toFinset, ZeroHom.toFun_eq_coe,
            AddMonoidHom.toZeroHom_coe, one_mul, Set.Finite.mem_toFinset, ne_eq,
            ENNReal.div_eq_zero_iff, Nat.cast_eq_zero, ENNReal.nat_ne_top, or_false,
            ← Nat.pos_iff_ne_zero, Nat.card_pos]
      have hx : IsUniform (A_ x) UA μx := sorry
      have hy : IsUniform (B_ y) UB μy := sorry
      rewrite [rdist_set_eq_rdist h_μx h_μy hx hy hUA' hUB', Set.preimage_comp, Set.preimage_comp]
      show _ = ((ℙ : Measure Ω).real _) * ((ℙ : Measure Ω').real _) * _
      rewrite [hUA_coe.measureReal_preimage _ hUA', hUB_coe.measureReal_preimage _ hUB']
      simp_rw [IsProbabilityMeasure.measureReal_univ, one_mul]
      rewrite [mul_div_mul_comm, Set.inter_comm A, Set.inter_comm B]
      congr <;> simp only [Set.Finite.coe_toFinset, Set.Finite.mem_toFinset, Finset.mem_val] <;> rfl
  ...
  have hM : M = ∑ x in X, ∑ y in Y, Real.negMulLog (p x y) := by
    have h_compl (z : H × H) (h : z ∉ X ×ˢ Y) : negMulLog (p z.1 z.2) = 0 := by
      rewrite [Finset.mem_product, not_and_or] at h
      unfold_let p; beta_reduce
      cases h with
      | inl h =>
        have : A_ z.1 = ∅ := by
          by_contra hc
          obtain ⟨_, ha⟩ := Set.nonempty_iff_ne_empty'.mpr hc
          obtain ⟨ω, hω⟩ :=
            hUA_coe.nonempty_preimage_of_mem hUA' <| ((Set.toFinite A).mem_toFinset).mpr ha.1
          exact h <| (FiniteRange.mem_iff _ z.1).mpr ⟨ω, congr_arg _ hω |>.trans ha.2⟩
        rw [this]
        simp? says
          simp only [Nat.card_eq_fintype_card, Fintype.card_of_isEmpty, CharP.cast_eq_zero,
            ZeroHom.toFun_eq_coe, AddMonoidHom.toZeroHom_coe, zero_mul, zero_div, negMulLog_zero]
      | inr h =>
        have : B_ z.2 = ∅ := by
          by_contra hc
          obtain ⟨_, hb⟩ := Set.nonempty_iff_ne_empty'.mpr hc
          obtain ⟨ω, hω⟩ :=
            hUB_coe.nonempty_preimage_of_mem hUB' <| ((Set.toFinite B).mem_toFinset).mpr hb.1
          exact h <| (FiniteRange.mem_iff _ z.2).mpr ⟨ω, congr_arg _ hω |>.trans hb.2⟩
        rw [this]
        simp? says
          simp only [ZeroHom.toFun_eq_coe, AddMonoidHom.toZeroHom_coe, Nat.card_eq_fintype_card,
            Fintype.card_of_isEmpty, CharP.cast_eq_zero, mul_zero, zero_div, negMulLog_zero]
    unfold_let M
    unfold entropy
    ...
    rw [← Finset.sum_product', ← tsum_eq_sum h_compl, ← measureEntropy_prod]
    ...
    repeat rewrite [Set.preimage_comp]
    rewrite [hUA_coe.measure_preimage _ hUA', hUB_coe.measure_preimage _ hUB']
  ...
  calc
    _ = ∑ x in X, ∑ y in Y, (p x y) * M * dᵤ[A_ x # B_ y] + M * d[φ.toFun ∘ UA # φ.toFun ∘ UB] := by
      simp only [hM, Finset.sum_mul, ← Finset.sum_add_distrib]
      refine Finset.sum_congr rfl <| fun x hx ↦ Finset.sum_congr rfl <| fun y hy ↦ ?_
  ...
  have : ∃ x : X, ∃ y : Y,
    ...
    let g (xy : H × H) := (p xy.1 xy.2) *
      (M * dᵤ[A_ xy.1 # B_ xy.2] + d[φ.toFun ∘ UA # φ.toFun ∘ UB] * -log (p xy.1 xy.2))
    have ineq : ∀ xy ∈ X ×ˢ Y, f xy < g xy := by
      intro xy h
      unfold_let p f g; beta_reduce
      haveI := h_Ax ⟨xy.1, (Finset.mem_product.mp h).1⟩
      haveI := h_By ⟨xy.2, (Finset.mem_product.mp h).2⟩
    ...
    let ω : Ω := Classical.choice (ProbabilityMeasure.nonempty ⟨ℙ, inferInstance⟩)
    let ω' : Ω' := Classical.choice (ProbabilityMeasure.nonempty ⟨ℙ, inferInstance⟩)
    have : Finset.Nonempty (X ×ˢ Y) := by
      use Prod.mk (φ.toFun (UA ω)) (φ.toFun (UB ω'))
      apply Finset.mem_product.mpr
      exact ⟨FiniteRange.mem (φ.toFun ∘ UA) ω, FiniteRange.mem (φ.toFun ∘ UB) ω'⟩
    have h_sumc := Finset.sum_lt_sum_of_nonempty this ineq
    unfold_let f at this; beta_reduce at h_sumc
    rewrite [← Finset.sum_mul] at h_sumc
    have : ∑ x in X ×ˢ Y, p x.1 x.2 = 1 := by
      have hx : ∑ x in X, (Nat.card (A_ x) : ℝ) / (Nat.card A) = 1 := sorry
      have hy : ∑ y in Y, (Nat.card (B_ y) : ℝ) / (Nat.card B) = 1 := sorry
      simp_rw [← hx, Finset.sum_product, mul_div_mul_comm, ← Finset.mul_sum, hy, mul_one]
    rewrite [this, one_mul] at h_sumc
    rw [← Finset.sum_product'] at h_sum
    exact not_le_of_gt h_sumc h_sum
  obtain ⟨x, y, hxy⟩ := this
