...
variable {Ω S T β : Type*} [mΩ : MeasurableSpace Ω]
  ...
  [Nonempty β] [Countable β] [MeasurableSpace β] [MeasurableSingletonClass β]
  [Nonempty γ] [Countable γ] [MeasurableSpace γ] [MeasurableSingletonClass γ]
...
lemma finiteKernelSupport_of_reverse {κ : kernel T (α × β × γ)} (hκ : FiniteKernelSupport κ) :
    FiniteKernelSupport (reverse κ) :=
  finiteKernelSupport_of_map hκ _

lemma AEFiniteKernelSupport.reverse {κ : kernel T (S × β × γ)} {μ : Measure T}
