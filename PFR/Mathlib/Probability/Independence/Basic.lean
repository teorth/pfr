...
lemma iIndepFun.iIndepFun_finsets {f : ∀ i, Ω → β i} {J : Type*} [Fintype J]
  ...
  exact kernel.iIndepFun.iIndepFun_finsets S h_disjoint hf_Indep hf_meas
...
lemma iIndepFun.iIndepFun_finsets_comp {f : ∀ i, Ω → β i} {J : Type*} [Fintype J]
    (S : J → Finset ι) (h_disjoint : Set.PairwiseDisjoint Set.univ S)
    (hf_Indep : iIndepFun m f μ) (hf_meas : ∀ i, Measurable (f i))
  ...
  have := hf_Indep.isProbabilityMeasure
  exact kernel.iIndepFun.iIndepFun_finsets_comp S h_disjoint hf_Indep hf_meas γ φ hφ
