...
lemma integral_eq_sum_finset' (μ : Measure α) [IsFiniteMeasure μ] [MeasurableSingletonClass α]
    {A : Finset α} (f : α → E) (hA : μ Aᶜ = 0) :
    ∫ x, f x ∂μ = ∑ x in A, (μ {x}).toReal • f x := by
...
lemma lintegral_eq_sum_finset' (μ : Measure α) [MeasurableSingletonClass α]
    {A : Finset α} (f : α → ℝ≥0∞) (hA : μ Aᶜ = 0) :
    ∫⁻ x, f x ∂μ = ∑ x in A, (f x) * (μ {x}) := by
  have hA' : (A : Set α) =ᵐ[μ] Set.univ := by rwa [MeasureTheory.ae_eq_univ]
  rw [← MeasureTheory.set_lintegral_univ, ← MeasureTheory.set_lintegral_congr hA']
  apply lintegral_finset

lemma lintegral_eq_single (μ : Measure α) [MeasurableSingletonClass α]
    (a : α) (f : α → ℝ≥0∞) (ha : ∀ b : α, b ≠ a → f b = 0) : ∫⁻ x, f x ∂μ = f a * μ {a} := by
